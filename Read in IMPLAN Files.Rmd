...
gf_point(pctindirect ~ B_sd,data=shock01, color="blue",
         title = "How Much Sectoral Shocks Propogate to the Aggregate Economy",
         subtitle = "As Production Networks are more Centralized",
         caption = "Source: IMPLAN Michigan County Data, 2014") %>%
  gf_lm(pctindirect ~ B_sd,data=shock01) 

lm(pctindirect ~ B_mean + B_sd,data=shock01) %>% stargazer(type="text")
...
IO.df  <- collapse_to_matrices(IOlong,matnames="matrix",matvals="vals") %>%
   ...
   mutate(g = (rowsums_byname(IO)),
          ...
          L = inverse_byname(Iminus_byname(A))
```
